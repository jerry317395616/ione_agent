...
def extend_bootinfo(bootinfo) -> None:
	"""Expose the I-ONE workspace in Frappe v17 Apps and Desktop Icons modes."""
	...
	app_data = [item for item in app_data if _app_name(item) != DIFY_APP_NAME]
	bootinfo["app_data"] = app_data
	if "desktop_icons" in bootinfo:
		desktop_icons = list(bootinfo.get("desktop_icons") or [])
		bootinfo["desktop_icons"] = [item for item in desktop_icons if not _is_workspace_launcher_icon(item)]
